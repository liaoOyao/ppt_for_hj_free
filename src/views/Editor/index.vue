<template>
  <!-- <div id="hj_ppt_load" style="text-align: center;height:calc(100% - 25px)">
        ...
        <div v-if="company_array.length > 0" class="hj_sidebar_category_ppt">
          ...
          <div v-for="(item, index) in dep_array" class="hj_sidebar_ppt_item " :key="index" :data-table_pk="item.pk"
            ...
            <a class="higet_ppt_pointer hj_siderbar_ppt_height hj_ppt_font "
              ...
              {{ item.name ||item.tetx? item.text:'无'}}
            </a>
          </div>
          <div v-if="bupl_array.length > 0 || spfd_array.length > 0" class="hj_slider_bottom_line"></div>
        </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    // const value_c_index = ref<number>(0)
    ...
    const handleUrlAndSleect = (url: string) => {
      ...
      if (urlObj && urlObj.search) {
        ...
        // 左侧列表和上侧下拉列表一样  除了公司
        // 接下来根据参数是否有获取数据信息
        const dim_data = dimension_list_data.dimension_obj
        try {
          // 标记要显示的  填充当前选中的
          if (params.has('spfd_id')) {
            ...
            if (temp && temp !== '-1') {
              ...
              spfd_id.value = temp
              value_spfd.value = Number(dim_data.spfd_id)
            }
          }
          // bupl
          if (params.has('bupl_id')) {
            ...
            if (temp && temp !== '-1') {
              ...
              bupl_id.value = temp
              value_bupl.value = Number(dim_data.pl_id)
            }
          }
          // d 部门
          if (params.has('d_id')) {
            ...
            if (temp && temp !== '-1') {
              ...
              d_id.value = temp
              value_dep.value = Number(dim_data.d_id)
            }
          }
          // 年份
          if (params.has('year')) {
    ...
    //  设置打开默认选中对象
    const set_default_select_item = (pk: any, pk_name: string, type: string) => {
    ...
    const get_dimession_list = () => {
      // 获取维度列表
      // 根据url 中的参数 发送到后端请求列表数据
    ...
    const open_loading = () => {
      loading.value = ElLoading.service({
        ...
        target: '.pptist-div'  // 这里添加你的div的class或者id
      })
    }
    const close_loading = () => {
      if (loading.value) {
